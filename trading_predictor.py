...
def calculate_advanced_indicators(data):
    """Calculate ultra-comprehensive technical indicators with robust NaN handling"""
    df = data.copy()
    ...
    try:
        ...
        # For any remaining NaN values, fill with 0 or appropriate value
        ...
        # Return original data with basic cleaning if advanced indicators fail
...
def advanced_signal_generation(data):
    ...
    atr = data['ATR'].iloc[-1] if 'ATR' in data.columns and not pd.isna(data['ATR'].iloc[-1]) else current_price * 0.01
    ...
    if 'RSI' in data.columns and not pd.isna(data['RSI'].iloc[-1]):
        rsi = data['RSI'].iloc[-1]
        rsi_prev = data['RSI'].iloc[-2] if not pd.isna(data['RSI'].iloc[-2]) else 50
    ...
    if 'MACD_Crossover' in data.columns and 'MACD_Histogram' in data.columns:
        macd_hist = data['MACD_Histogram'].iloc[-1] if not pd.isna(data['MACD_Histogram'].iloc[-1]) else 0
        macd_cross = data['MACD_Crossover'].iloc[-1] if not pd.isna(data['MACD_Crossover'].iloc[-1]) else 0
    ...
    if 'Volume_Ratio' in data.columns and 'OBV_Trend' in data.columns:
        vol_ratio = data['Volume_Ratio'].iloc[-1] if not pd.isna(data['Volume_Ratio'].iloc[-1]) else 1
        obv_trend = data['OBV_Trend'].iloc[-1] if not pd.isna(data['OBV_Trend'].iloc[-1]) else 0
    ...
    if 'VWAP' in data.columns and not pd.isna(data['VWAP'].iloc[-1]):
        ...
        price_vs_vwap = data['Price_vs_VWAP'].iloc[-1] if not pd.isna(data['Price_vs_VWAP'].iloc[-1]) else 0
    ...
    if 'BB_Position' in data.columns and 'BB_Squeeze' in data.columns:
        bb_pos = data['BB_Position'].iloc[-1] if not pd.isna(data['BB_Position'].iloc[-1]) else 0.5
        bb_squeeze = data['BB_Squeeze'].iloc[-1] if not pd.isna(data['BB_Squeeze'].iloc[-1]) else 0
    ...
    if 'MFI' in data.columns and not pd.isna(data['MFI'].iloc[-1]):
        mfi = data['MFI'].iloc[-1]
...
def ensemble_ml_prediction(data, minutes_ahead=15):
    """Ensemble machine learning prediction with multiple models - Robust NaN handling"""
    try:
        if len(data) < 50:
        ...
        # Use HistGradientBoostingRegressor which handles NaN natively
        X_gb = feature_data.iloc[:-1].values
        y_gb = feature_data['Close'].shift(-1).iloc[:-1].values
        
        # Remove any rows with NaN in target
        valid_indices = ~np.isnan(y_gb)
        X_gb = X_gb[valid_indices]
        y_gb = y_gb[valid_indices]
        ...
        for step in range(minutes_ahead):
            ...
            if step == 0:
                ...
                # Check for NaN in last features
        ...
        for i in range(minutes_ahead):
            if i < len(gb_predictions) and gb_predictions:
        ...
        if len(ensemble_predictions) > 0 and np.mean(ensemble_predictions) > 0:
            pred_volatility = np.std(ensemble_predictions) / np.mean(ensemble_predictions)
            confidence = max(0.55, min(0.92, 1 - pred_volatility * 4))
        else:
            confidence = 0.7
        
        # Model agreement score
        if gb_predictions and lstm_predictions and lstm_predictions[-1] != 0:
            agreement = 1 - (abs(lstm_predictions[-1] - gb_predictions[0]) / abs(lstm_predictions[-1]))
...
symbol = st.sidebar.text_input("Stock Symbol", value="^NSEBANK").upper()
interval = st.sidebar.selectbox("Time Interval", ["1m", "2m", "5m", "15m", "30m"], index=0)
...
if data is not None and not data.empty:
    ...
    with st.spinner('🧠 Calculating 40+ technical indicators with robust data cleaning...'):
    ...
    # ML Prediction
    with st.spinner('🤖 Ensemble ML prediction in progress (with NaN-safe models)...'):
        try:
...
- NaN-safe models ensure robust predictions
""")